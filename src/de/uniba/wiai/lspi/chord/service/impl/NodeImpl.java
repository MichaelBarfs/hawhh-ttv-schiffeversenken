...
public final class NodeImpl extends Node {
	...
	public final void broadcast(Broadcast info) throws CommunicationException {
		...
		if(info.getTransaction() <= TransactionHelper.transactionNumber){
			return;
		} else {
			TransactionHelper.transactionNumber = info.getTransaction();
		}

		// get nodes of finger table.
		List<Node> originalFingerTable = impl.getFingerTable();

		// remove duplicates.
		List<Node> fingerTable = new ArrayList<Node>();
		for (Node node : originalFingerTable) {
			//check if node already in send table
			if (!fingerTable.contains(node)) {
				//check if node is in range.
				if(node.getNodeID().isInInterval(impl.getID(), info.getRange())){//maybe this is buggy?
					fingerTable.add(node);
				}
			}
		}

		// send broadcast to all nodes in finger tabel
